...
def eval_score(query_data: str, tested_plan: str):
    if type(query_data) == str:
        query_data = eval(query_data)
    if type(tested_plan) == str:
        tested_plan = eval(tested_plan)
    ...
    plan_constraint_store = {'commonsense_constraint':commonsense_info_box,'hard_constraint':hard_info_box}


    # We evaluate the hard constraint pass rate
    score = 0
    explanation = ""
    if plan_constraint_store['commonsense_constraint']:
        final_commonsense_pass = True
        final_hardConstraint_pass = True
        for item in plan_constraint_store['commonsense_constraint']:
            if plan_constraint_store['commonsense_constraint'][item][0] is not None and not plan_constraint_store['commonsense_constraint'][item][0]:
                explanation = plan_constraint_store['commonsense_constraint'][item][1]
                final_commonsense_pass = False
                break
        if plan_constraint_store['hard_constraint']:
            score = 1
            for item in plan_constraint_store['hard_constraint']:
                if plan_constraint_store['hard_constraint'][item][0] is not None and  plan_constraint_store['hard_constraint'][item][0] == False:
                    explanation = plan_constraint_store['hard_constraint'][item][1]
                    final_hardConstraint_pass = False
                    score = 0
                    break
